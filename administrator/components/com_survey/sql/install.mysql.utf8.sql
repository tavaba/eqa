 ...
 * Version 1.0.1
 */
CREATE TABLE IF NOT EXISTS `#__survey_logs`(
	`id`			INT AUTO_INCREMENT,
	`user_id`		INT NOT NULL		COMMENT 'Linked to #__users',			
	`action`		INT NOT NULL		COMMENT 'Code of action type',			
	`entity_type`	INT NOT NULL		COMMENT 'Code of entity type',
	`entity_id`		INT,
	`result`		INT													COMMENT 'Code of result type',
	`data`			TEXT												COMMENT 'A JSON containing additional info if available',
	`log_date`		DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP			COMMENT 'As the Joomla Core action log table naming practice',
    PRIMARY KEY (`id`),
	INDEX `idx_logs_user` (`user_id`),
	INDEX `idx_logs_action` (`action`),
	INDEX `idx_logs_entity` (`entity_type`)
