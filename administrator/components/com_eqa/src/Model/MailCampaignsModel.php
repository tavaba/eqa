...
use Kma\Library\Kma\Constant\HtmlTag;
...
class MailCampaignsModel extends BaseMailCampaignModel
{
    ...
    // =========================================================================
    // Abstract override — cung cấp context label theo nghiệp vụ com_eqa
    // =========================================================================

    /**
     * Trả về nhãn ngữ cảnh để lưu vào DB tại thời điểm tạo campaign.
     * Được gọi bởi MailCampaignsController (lib_kma) trước khi gọi MailService::notify().
     *
     * @param  int  $contextType  Giá trị MailContextType enum
     * @param  int  $contextId    ID đối tượng ngữ cảnh
     *
     * @return string  Ví dụ: 'Toán cao cấp 1 (HK1/2024-2025)'
     * @since  2.0.8
     */
    ...
    private function resolveExamRecipients(int $examId, ?string $recipientFilter): array
    {
        ...
        foreach ($recipients as &$recipient) {
			...
			{
				$recipient->allowed_to_exam = HtmlTag::SPAN_BADGE_SUCCESS . 'QUY ĐỔI ĐIỂM</span>'
			...
			{
				$recipient->allowed_to_exam = HtmlTag::SPAN_BADGE_SUCCESS . 'MIỄN THI</span>'
					. ' (' . $recipient->stimulation_value . ' điểm)';
				$recipient->examsession = '';
			}
			elseif($allowed)
	        {
		        $recipient->allowed_to_exam = HtmlTag::SPAN_BADGE_SUCCESS . 'ĐƯỢC THI</span>';
			...
			else
			{
				...
				$recipient->allowed_to_exam = HtmlTag::SPAN_BADGE_DANGER . 'KHÔNG ĐƯỢC THI</span>';
    ...
    private function resolveExamSeasonRecipients(int $examSeasonId, ?string $recipientFilter): array
    {
		...
		$columns = [
			...
			$db->quoteName('es.name',  'examseason_name'),
			$db->quoteName('es.ppaa_req_deadline'),
		];
        $query = $db->getQuery(true)
            ->select($columns)
            ->from('#__eqa_exam_learner AS el')
	        ->leftJoin('#__eqa_learners AS l', 'l.id = el.learner_id')
	        ->leftJoin('#__eqa_exams AS e', 'e.id = el.exam_id')
	        ->leftJoin('#__eqa_examseasons AS es', 'es.id = e.examseason_id')
            ->where('e.examseason_id = ' . $examSeasonId)
	    ...
	    /**
	     * Bổ sung các thuộc tính còn thiếu. Convert thời gian sang local time.
	     * - Các thuộc tính bắt buộc có: type, email
	     * - Các thuộc tính cho placeholder: fullname, ppaa_req_deadline
	     */
		$mailService = $this->getMailService();
        foreach ($recipients as &$recipient)
		{
			...
			if(empty($recipient->ppaa_req_deadline))
				$recipient->ppaa_req_deadline = '<b>Công bố sau</b>';
			else
				$recipient->ppaa_req_deadline = '<b>' . DatetimeHelper::convertToLocalTime($recipient->ppaa_req_deadline) . '</b>';
        }
