...
/**
 * Model danh sách người học có trong bảng #__eqa_resit_learner,
 ...
 * @since 2.0.5
 */
class ResitLearnersModel extends ListModel
{
    ...
    public function getListQuery()
    {
        ...
        // Thống kê trong phạm vi MỘT danh sách thi lần 2 (2.1.8)
        $resitId = $this->getState('filter.resit_id');

        if (!is_numeric($resitId) || (int) $resitId <= 0) {
            throw new \RuntimeException('Không xác định được danh sách thi lần 2.');
        }
        ...
        $query = $db->getQuery(true)
            ...
            // Giới hạn theo danh sách thi lần 2 đang xem (2.1.8)
            ->where($db->quoteName('sa.resit_id') . ' = ' . (int) $resitId);
    ...
    public function getStoreId($id = '')
    {
        ...
        //Khóa cache phải chứa danh sách đang xem, nếu không hai danh sách khác
        //nhau sẽ dùng chung kết quả đã cache (2.1.8)
        $id .= ':list' . (int) $this->getState('filter.resit_id');
