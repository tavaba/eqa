...
class ExamroomsController extends EqaAdminController {
	...
	public function import_bak(): void
	{
	...
	public function import(): void
	{
		...
		try
		{
			...
			//retrieve form data
			$importAnomaly = $app->input->getBool('import_anomaly');
			if(is_null($importAnomaly))
				throw new Exception('Dữ liệu không hợp lệ');
			$files = $this->input->files;
			$file = $files->get($fileFormField);
			if(empty($file['tmp_name']))
				throw new Exception(Text::_('COM_EQA_MSG_ERROR_NO_FILE_UPLOADED'));

			//Mở file để đọc
			$examroomIdPattern = '/^Ngày thi: [\s\S]+ Mã phòng thi: ([0-9]+)$/';
		...
		for($sh=0; $sh<$sheetNumber; $sh++){
			...
			for(; $row <= $highestRow; $row++)
			{
				...
				if(empty($cellValue))
					continue;
				$matched = preg_match($examroomIdPattern, $cellValue, $matches);
				if($matched){
					$examroomId = (int)$matches[1];
					break;
				}
